...
Use the [site i18n configuration](docs/api/docusaurus.config.js.md#i18n) to declare the i18n locales:
...
For any React code you've written yourself: React pages, React components, etc., you will use the [**translation APIs**](docs/docusaurus-core.md#translate).
...
Docusaurus provides a **very small and lightweight translation runtime** on purpose, and only supports basic [placeholders interpolation](docs/docusaurus-core.md#interpolate), using a subset of the [ICU Message Format](https://formatjs.io/docs/core-concepts/icu-syntax/).
...
Run the [write-translations](docs/cli.md#docusaurus-write-translations-sitedir) command:
...
Notice that the `docusaurus-plugin-content-docs` plugin always divides its content by versions. The data in `./docs` folder will be translated in the `current` subfolder and `current.json` file. See [the doc versioning guide](docs/guides/docs/versioning.md#terminology) for more information about what "current" means.
