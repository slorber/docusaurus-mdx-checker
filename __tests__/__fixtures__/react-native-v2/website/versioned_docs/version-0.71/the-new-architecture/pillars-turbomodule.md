...
If you've worked with React Native, you may be familiar with the concept of [Native Modules](versioned_docs/version-0.71/native-modules-intro.md), which allow JavaScript and platform-native code to communicate over the React Native "bridge", which handles cross-platform serialization via JSON.
...
Next, you need to add some configuration for [**Codegen**](versioned_docs/version-0.71/the-new-architecture/pillars-codegen.md) and auto-linking.
...
As the first step for both iOS and Android, this guide shows how to execute manually the scripts used by **Codegen** to generate the required code. Further information on **Codegen** can be found [here](versioned_docs/version-0.71/the-new-architecture/pillars-codegen.md).
