...
If you need section support, use [`<SectionList>`](versioned_docs/version-0.72/sectionlist.md).
...
To render multiple columns, use the [`numColumns`](versioned_docs/version-0.72/flatlist.md#numcolumns) prop. Using this approach instead of a `flexWrap` layout can prevent conflicts with the item height logic.
...
This is a convenience wrapper around [`<VirtualizedList>`](versioned_docs/version-0.72/virtualizedlist.md), and thus inherits its props (as well as those of [`<ScrollView>`](versioned_docs/version-0.72/scrollview.md)) that aren't explicitly listed here, along with the following caveats:
...
Inherits [ScrollView Props](versioned_docs/version-0.72/scrollview.md#props), unless it is nested in another FlatList of same orientation.
...
An array (or array-like list) of items to render. Other data types can be used by targetting [`VirtualizedList`](versioned_docs/version-0.72/virtualizedlist.md) directly.
