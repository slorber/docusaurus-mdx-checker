...
- **Named exports**: the [static methods](docs/api/plugin-methods/static-methods.md) called before plugins are initialized.
...
`options` are the [second optional parameter when the plugins are used](docs/using-plugins.md#configuring-plugins). `options` are plugin-specific and are specified by users when they use them in `docusaurus.config.js`. If there's a [`validateOptions`](docs/api/plugin-methods/static-methods.md#validateOptions) function exported, the `options` will be validated and normalized beforehand.
